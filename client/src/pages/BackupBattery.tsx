...
const BatteryHistoryDashboard: React.FC<BatteryHistoryDashboardProps> = ({
  ...
  const [timeRange, setTimeRange] = useState<"6h" | "24h" | "7d" | "30d">(
    "24h"
  );
  ...
  const exportData = () => {
    ...
    a.setAttribute(
      "download",
      `battery-history-${deviceId}-${new Date().toISOString().slice(0, 10)}.csv`
    );
  ...
  return (
    <div
      className={`p-4 md:p-6 space-y-6 min-h-screen transition-colors duration-300 ${
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap gap-2">
          <div
            className={`flex space-x-1 rounded-lg shadow p-1 ${
      ...
      <AnimatePresence>
        {error && (
          <motion.div
            ...
            className={`border-l-4 border-red-500 p-4 rounded ${
      ...
      >
        <motion.div
          ...
          className={`shadow rounded-lg p-5 transition-colors duration-300 ${
            darkMode ? "bg-gray-800" : "bg-white"
          }`}
        >
        ...
        >
          <div className="flex items-center">
            ...
            >
              <Clock
                className={`h-5 w-5 ${getStatusColor(latest.percentage)}`}
              />
            </div>
            <div className="ml-5">
              <p
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className={`shadow rounded-lg p-4 md:p-6 transition-colors duration-300 ${
          darkMode ? "bg-gray-800" : "bg-white"
        }`}
      >
        <h2
        ...
        {[
          {
            key: "voltage",
            label: "Voltage (V)",
            color: darkMode ? "#3b82f6" : "#0284c7",
            bgColor: darkMode ? "bg-blue-900/30" : "bg-blue-50",
          },
          {
            key: "current",
            label: "Current (A)",
            color: darkMode ? "#d946ef" : "#c026d3",
          ...
          <motion.div
            key={key}
            variants={itemVariants}
            className={`shadow rounded-lg p-4 transition-colors duration-300 ${
              darkMode ? "bg-gray-800" : "bg-white"
            }`}
          >
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  <CartesianGrid
                    strokeDasharray="3 3"
                    opacity={0.2}
                    stroke={darkMode ? "#4a5568" : undefined}
                  />
                  <XAxis
                    dataKey="time"
                    tick={{
      ...
      <motion.div
        ...
        initial="hidden"
        animate="visible"
        className={`shadow rounded-lg overflow-hidden transition-colors duration-300 ${
          darkMode ? "bg-gray-800" : "bg-white"
        }`}
      >
        <div
        ...
        <div className="overflow-x-auto">
          ...
          >
            ...
            >
              <AnimatePresence>
                {sorted
                  .slice(Math.max(sorted.length - 15, 0))
                  .reverse()
                  .map((entry, index) => (
                    <motion.tr
                      key={entry._id}
          ...
          {sorted.length > 15 && (
            <motion.div
              ...
              className={`px-6 py-3 border-t text-center ${
                darkMode ? "border-gray-700" : "border-t"
              }`}
            >
              <motion.button
                whileHover={{ scale: 1.05 }}
